		   ...
		   SolutionInfo* si, int shrinking)
{
	...
	for (int i = 0; i < l; ++i)
		C_[i] = (y_[i] > 0 ? Cp : Cn);
	Solve(l, Q, p_, y_, alpha_, C_, eps, si, shrinking);
	si->upper_bound_p = Cp;
	si->upper_bound_n = Cn;
	delete[] C_;
}
...
public:
	...
	void Solve(int l, const QMatrix& Q, const double *p, const schar *y,
		   double *alpha, const double* C, double eps,
		   SolutionInfo* si, int shrinking)
	{
		this->si = si;
		Solver::Solve(l,Q,p,y,alpha,C,eps,si,shrinking);
	}
	...
	double *alpha, Solver::SolutionInfo* si, const double *C_)
{
	...
	double nul = 0;
	for(i=0;i<l;i++) nul += nu*C_[i];

	double sum_pos = nul/2;
	double sum_neg = nul/2;

	for(i=0;i<l;i++)
		if(y[i] == +1)
		{
			alpha[i] = min(1.0*C_[i],sum_pos);
			sum_pos -= alpha[i];
		}
		else
		{
			alpha[i] = min(1.0*C_[i],sum_neg);
			sum_neg -= alpha[i];
		}
	...
	double *alpha, Solver::SolutionInfo* si,const double* C_)
{
	...
	double sum = 0.0;
	for(i=0;i<l;i++) sum += C_[i] * nu;
	
	for(i=0;i<l;i++)
	{
		alpha[i] = min(1.0*C_[i],sum);
		sum -= alpha[i];
	}
	...
	double *alpha, Solver::SolutionInfo* si,const double *C_)
{
	...
	for(i=0;i<l;i++)
	{
		alpha2[i] = alpha2[i+l] = min(sum,C_[i]);
		sum -= alpha2[i];
	...
	double Cp, double Cn)
{
	...
	}else if(param->svm_type == NU_SVC || param->svm_type == ONE_CLASS){
		C_ = new double[prob->l];
		for(int i=0;i<prob->l;i++) C_[i] = 1.0;
	}
	...
	switch(param->svm_type)
	{
		case C_SVC:
			...
			break;
		case NU_SVC:
			solve_nu_svc(prob,param,alpha,&si,C_);
			break;
		case ONE_CLASS:
			solve_one_class(prob,param,alpha,&si,C_);
			break;
		case EPSILON_SVR:
			solve_epsilon_svr(prob,param,alpha,&si,C_);
			break;
		case NU_SVR:
			solve_nu_svr(prob,param,alpha,&si,C_);
			break;
	}
...
//
// Remove zero weighed data. The result is in prob_ret
//
static void svm_weight_remove_zero(svm_problem* prob_ret,const svm_problem* prob){
	int i;
	int l = 0;
	double *W = NULL;
	if (prob->W) W = Malloc(double, prob->l);
	svm_node **x = Malloc(svm_node *,prob->l);
	double *y = Malloc(double, prob->l);
	for(i=0;i<prob->l;i++)
	{
		if(prob->W && prob->W[i] == 0) continue;
		if(W) W[l] = prob->W[i];
		x[l] = prob->x[i];
		y[l] = prob->y[i];
		l++;
	...
	prob_ret->l = l;
	prob_ret->x = Malloc(svm_node*,l);
	prob_ret->y = Malloc(double,l);
	if (W) prob_ret->W = Malloc(double, l);
	else prob_ret->W = NULL;
	for(i=0;i<l;i++)
	{
		prob_ret->x[i] = x[i];
		prob_ret->y[i] = y[i];
		if (W) prob_ret->W[i] = W[i];
	}
...
// Interface functions
//
svm_model *svm_train(const svm_problem *prob_, const svm_parameter *param)
{
	svm_model *model = Malloc(svm_model,1);
	svm_problem *prob = Malloc(svm_problem,1);
	svm_weight_remove_zero(prob,prob_);
	model->param = *param;
	model->free_sv = 0;	// XXX
	...
	if(svm_type == NU_SVC)
	{
		...
		for(i=0;i<l;i++)
		{
			...
			if(j == nr_class)
			{
				...
				label[nr_class] = this_label;
				if(prob->W) count[nr_class] = prob->W[i];
				else count[nr_class] = 1;
				++nr_class;
			}
		}
